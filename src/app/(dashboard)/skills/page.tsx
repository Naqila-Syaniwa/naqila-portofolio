...
export default function SkillsPage() {
    ...
    function renderCard(category: SkillCategory) {
    ...
    return (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible">
            <div className="flex flex-col gap-6 sm:hidden">
                {skillCategories.map(renderCard)}
            </div>

            <div className="hidden gap-6 sm:flex sm:items-start">
                <div className="flex flex-1 flex-col gap-6">{leftColumn.map(renderCard)}</div>
                <div className="flex flex-1 flex-col gap-6">{rightColumn.map(renderCard)}</div>
            </div>
        </motion.div>
    );
}